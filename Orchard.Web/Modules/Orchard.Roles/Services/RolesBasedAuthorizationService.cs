...
namespace Orchard.Roles.Services
{
    public class RolesBasedAuthorizationService : IAuthorizationService
    {
        ...
        public RolesBasedAuthorizationService(IRoleService roleService,
            ...
            IWorkContextAccessor workContextAccessor, IAuthorizationServiceEventHandler authorizationServiceEventHandler)
        {
        ...
        public void CheckAccess(Permission permission, IUser user)
        {
            ...
            {
                throw new OrchardSecurityException("A security exception occurred in the content management system.")
                {
        ...
        public bool TryCheckAccess(Permission permission, IUser user)
        {
            ...
            for (var adjustmentLimiter = 0; adjustmentLimiter != 3; ++adjustmentLimiter)
            {
                ...
                if (!context.Granted)
                {
                    ...
                    {
                        var userRoles = _userRolesRepository.Fetch(x => x.UserId == user.Id).Select(x => x.Role.Name).ToList();

                        if (!userRoles.Contains(AnonymousRole[0]))
                        {
                            rolesToExamine = userRoles.Concat(AuthenticatedRole);
                        }
                        else
