    ...
    public class DataMigrationNotificationProvider : INotificationProvider
    {
        ...
        public IEnumerable<NotifyEntry> GetNotifications()
        {
            ...
            if (features.Any())
            {
                ...
                {
                    Message = string.Format("Some features need to be upgraded: {0}",
                       string.Join(", ", features
                        .Select(feature =>
                        string.Format("<a href=\"{0}#{1}\">{2}</a>", urlHelper.Action("Features", "Admin", new RouteValueDictionary { { "area", "Orchard.Modules" } }), "", feature)))),
