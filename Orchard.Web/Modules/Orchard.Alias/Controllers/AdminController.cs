    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Add(string aliasPath, string routePath)
        {
            ...
            catch (Exception ex)
            {
                ...
                Services.Notifier.Error(string.Format("An error occured while creating the alias {0}: {1}. Please check the values are correct.", aliasPath, ex.Message));
        ...
        public ActionResult Edit(string path, string aliasPath, string routePath)
        {
            ...
            {
                Services.TransactionManager.Cancel();
                Services.Notifier.Error(string.Format("An error occured while editing the alias '{0}': {1}. Please check the values are correct.", aliasPath, ex.Message));
                Logger.Error(ex, T("An error occured while creating the alias '{0}'", aliasPath).Text);
            ...
            Services.Notifier.Information(string.Format("Alias {0} updated", path));
        ...
        public ActionResult Delete(string path, string returnUrl)
        {
            ...
            Services.Notifier.Information(string.Format("Alias {0} deleted", path));
        ...
        private bool CheckAndWarnIfAliasExists(string aliasPath)
        {
            ...
            Services.Notifier.Error(string.Format("Cannot save alias <i>{0}</i>. It conflicts with existing one pointing to {1}. Please {2} or {3} the existing alias first.",
                aliasPath,
                routePathLink,
                changeLink,
                deleteLink));
